...
```{r}
d <- occ_download_get("0154158-230224095556074", path = output_path, overwrite = TRUE) %>%
  occ_download_import()
```
...
# Write data to csv
```{r}
TropicalAndes_GBIF_path <- file.path(output_path,"TropicalAndes_GBIF_frugivore_occ.csv") 
write.csv(d, file = TropicalAndes_GBIF_path)
TropicalAndes_GBIF_species_path <- file.path(output_path,"TropicalAndes_GBIF_frugivore_occ_species.csv") 
