...
na_summary(Ecuador_POWO, exclude_cols = c("accepted", "url", "images"))
```

```{r}
# using DataExplorer package
plot_str(Ecuador_POWO)
introduce(Ecuador_POWO)
plot_intro(Ecuador_POWO)
plot_missing(Ecuador_POWO)
plot_bar(Ecuador_POWO)
```

```{r}
# using summarytools package
Ecuador_POWO_summary <- dfSummary(Ecuador_POWO, 
            na.col       = FALSE,
            style        = "multiline",
            plain.ascii  = TRUE,
            graph.magnif = .8)
print(Ecuador_POWO_summary, method = "render")
view(Ecuador_POWO_summary, file = file.path(output_path,"Ecuador_POWO_summary.html"))
```


# Write data to csv
```{r}
#convert Ecuador_POWO from tbl_df to dataframe
Ecuador_POWO <- as.data.frame(Ecuador_POWO)
# remove image column from dataframe
Ecuador_POWO <- Ecuador_POWO %>% select(-images)
