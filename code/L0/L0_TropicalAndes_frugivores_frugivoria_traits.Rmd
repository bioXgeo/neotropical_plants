...
col_class <- function(x){
 ...
 cols <- colnames(x)
 numeric_cols <- c("diet_breadth", "body_mass_e", "body_size_mm", "longevity", "home_range_size", "generation_time", "habitat_breadth", "mean_CHELSA_bio1_1981.2010_V.2.1", "mean_CHELSA_bio12_1981.2010_V.2.1", "mean_human_fp_range_2010", "percent_change_hf_2010_2020","inferred_range_sqkm")
...
d <- col_class(Frug_mammal)

# also, replacing NA strings might not be necessary since there are none:
d == "NA"
which(d == "NA", arr.ind = TRUE)  

# need to double-check but function could replace what's below (and could adjust accordingly)
...
# Write data to csv
```{r}
write.csv(Frugivoria, file = file.path(output_path_L0,"TropicalAndes_Frugivoria_frugivore_traits_21Aug2025.csv"))
write.csv(Frugivoria_SpeciesList, file = file.path(output_path_L0,"TropicalAndes_Frugivoria_frugivore_traits_species_21Aug2025.csv"))
write.csv(Frugivore_LookupTable, file = file.path(output_path_L1,"TropicalAndes_frugivore_LookupTable_21Aug2025.csv"))
```
