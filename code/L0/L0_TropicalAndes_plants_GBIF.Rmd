...
```{r}
d1 <- occ_download_get("0009835-231002084531237", path = output_path, overwrite = TRUE) %>%
  occ_download_import()
```
...
```{r}
TropicalAndes_plant_occ_forest <- TropicalAndes_plant_occ[Forest_sf,]
```
...
nrow(TropicalAndes_plant_occ_forest)
print("The number of species is" )
length(unique(TropicalAndes_plant_occ_forest$species))
print("The number of genera is" )
length(unique(TropicalAndes_plant_occ_forest$genus))
print("The number of families is" )
length(unique(TropicalAndes_plant_occ_forest$family))
```
