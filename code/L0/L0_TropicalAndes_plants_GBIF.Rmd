...
```{r}
GBIF_SpeciesList <- unique(d2$species)
```
...
# Subset to Tropical Andes Lowland and Montane Forest
```{r}
...
coordinates(d) <- ~decimalLatitude+decimalLongitude
crs(d) <- crs(TropicalAndes_IUCNHabitat_Forest)
```

```{r}
GBIF_Forest <- d %>% filter(TropicalAndes_IUCNHabitat_Forest)
GBIF_Forest
plot(GBIF_Forest)
```
