...
TropicalAndes_GBIF_plant_occ_cleaned
TropicalAndes_frugivore_occ
```

# Crop occurrence data to forest extent
```{r}
TropicalAndes_GBIF_plant_occ_cleaned$ref<- extract(TropicalAndes_IUCNHabitat_Forest, TropicalAndes_GBIF_plant_occ_cleaned)
TropicalAndes_frugivore_occ$ref<- extract(TropicalAndes_IUCNHabitat_Forest, TropicalAndes_frugivore_occ)
```

```{r}
TropicalAndes_GBIF_plant_occ_cleaned$ref <- as.character(TropicalAndes_GBIF_plant_occ_cleaned$ref)
TropicalAndes_frugivore_occ$ref <- as.character(TropicalAndes_frugivore_occ$ref)
```


```{r}
plant_occ_forest <- TropicalAndes_GBIF_plant_occ_cleaned@data
frugivore_occ_forest <- TropicalAndes_frugivore_occ@data
```

```{r}
#remove row if ref = NA
plant_occ_forest <- plant_occ_forest[plant_occ_forest$ref != "NA", ]
frugivore_occ_forest <- frugivore_occ_forest[frugivore_occ_forest$ref != "NA", ]
```
