...
```{r}
# add to species dataframe
na_species_df_2 <- na_species_df %>%
  mutate(family = coalesce(family, taxonomic_df$family),
         genus = coalesce(genus, taxonomic_df$genus))

# retrieve list of species without family & genus information
species_no_family_2 <- na_species_df_2 %>%
  filter(is.na(family) | family == "") %>%
  select(species) %>%
  pull()
```
