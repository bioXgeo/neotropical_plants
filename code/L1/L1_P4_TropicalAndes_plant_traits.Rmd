...
numeric_data_average <- function(data, unit, traitname) {
  ...
  # Calculate average value, count, and variance per trait per species
  summary_data <- data %>%
    group_by(Accepted_species) %>%
    summarise(
      TraitValue_mean = mean(TraitValue, na.rm = TRUE),
      records_used = n(),
...
# Combine all traits
```{r}
trait_dfs <- list(averagetraits_fruitLength, averagetraits_fruitMass, averagetraits_plantHeight, averagetraits_plantLifespan, averagetraits_seedLength, averagetraits_seedMass, averagetraits_seedWidth, averagetraits_dispersal, averagetraits_growthForm)
