...
create_rich_plots <- function(resolution_meters) {
  ...
  gridPlot <- ggplot() +
    ...
    geom_sf(data = TAGrid) +
    labs(title = paste("Tropical Andes Grid [", resolution_meters/1000, " km]", sep=""), x = "Latitude", y = "Longitude") +
    coord_sf(xlim = c(-85, -54), ylim = c(-24, 14), expand = FALSE, crs = 4326) +
    annotation_scale(location = "bl", width_hint = 0.5) +
    annotation_north_arrow(location = "bl", which_north = "true", pad_x = unit(0.3, "in"), 
                           pad_y = unit(0.3, "in"), style = north_arrow_fancy_orienteering) +
    theme(panel.background = element_rect(fill = "lightblue"))
  ...
  # Plot richness for plants
  plantgridRichTA <- ggplot(plant_richness_grid) +
    geom_sf(data = Americas, fill = "white") +
    geom_sf(data = TApoly, fill = "grey", size = 0.1) +
    geom_sf(aes(fill = num_species), color = NA) +
    scale_fill_scico(palette = "romaO", direction = 1) +
    labs(title = paste("Tropical Andes Plant Richness [", resolution_meters/1000, " km]", sep=""), x = "Longitude", y = "Latitude",
  ...
  print(plantgridRichTA)

  # Plot richness for frugivores
  frugivoregridRichTA <- ggplot(frugivore_richness_grid) +
    geom_sf(data = Americas, fill = "white") +
    geom_sf(data = TApoly, fill = "grey", size = 0.1) +
    geom_sf(aes(fill = num_species), color = NA) +
    scale_fill_scico(palette = "romaO", direction = 1) +
    labs(title = paste("Tropical Andes Frugivore Richness [", resolution_meters/1000, " km]", sep=""), x = "Longitude", y = "Latitude",
         fill = "Number of species") +
    coord_sf(xlim = c(-85, -54), ylim = c(-24, 14), expand = FALSE, crs = 4326) +
    annotation_scale(location = "bl", width_hint = 0.5) +
    annotation_north_arrow(location = "bl", which_north = "true", 
  ...
  print(frugivoregridRichTA)

  # Extract cell values
  plant_cellRichness <- plantgridRichTA[["data"]]
  frugivore_cellRichness <- frugivoregridRichTA[["data"]]

  ### Plant vs frugivore richness plot
  compare_richness_summary <- summary(lm(frugivore_cellRichness$num_species ~ plant_cellRichness$num_species))
  print(compare_richness_summary)
  
  # save r-squared value
  compare_richness_r2 <- compare_richness_summary$r.squared
  
  plant_frug_richness_compare <- plot(plant_cellRichness$num_species, frugivore_cellRichness$num_species,
       main = paste("Plant Richness vs Frugivore Richness [", resolution_meters/1000, " km]", sep = ""),
       xlab = "Plant Richness by cell", ylab = "Frugivore Richness by cell")
  abline(lm(frugivore_cellRichness$num_species ~ plant_cellRichness$num_species))
  
  print(plant_frug_richness_compare)
  
  plant_richness_hist <- hist(plant_cellRichness$num_species,
       main = paste("Histogram of Plant Richness [", resolution_meters/1000, " km] by cell", sep = ""),
       xlab = "Plant Richness by cell")
  print(plant_richness_hist)
  frugivore_richness_hist <- hist(frugivore_cellRichness$num_species,
       main = paste("Histogram of Frugivore Richness [", resolution_meters/1000, " km] by cell", sep = ""),
       xlab = "Frugivore Richness by cell")
  print(frugivore_richness_hist)
}
...
# save plots
plantgridRichTA_1km <- plantgridRichTA
...
compare_richness_r2_1km <- compare_richness_r2
```
