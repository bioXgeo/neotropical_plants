 ...
 # Make Grid
  ...
  generate_plant_plot <- function(data) {
    ggplot(data) +
      ...
      scale_fill_distiller(palette = "Greens", limits = c(0,6000)) +
  ...
  generate_frugivore_plot <- function(data) {
    ggplot(data) +
      ...
      geom_sf(data = TApoly, fill = "grey", size = 0.1) +
      geom_sf(aes(fill = num_species), color = NA) +
      scale_fill_distiller(palette = "YlOrBr", limits = c(0,500)) + 
      labs(title = title, x = "Longitude", y = "Latitude", fill = "Number of species") +
      coord_sf(xlim = c(-82, -60), ylim = c(-24, 14), expand = FALSE, crs = 4326) +
      annotation_scale(location = "bl", width_hint = 0.5)  +
      theme(panel.background = element_rect(fill = "lightblue"))
  ...
  compare_richness_exponential_summary <- summary(lm(exp(comparison_data_filtered$Frugivore_Richness) ~
                                                       comparison_data_filtered$Plant_Richness))
