 ...
 # Make Grid
  ...
  generate_plant_plot <- function(data) {
    ggplot(data) +
      ...
      scale_fill_distiller(pallete = "Greens", limits = c(0,6000)) +
  ...
  generate_frugivore_plot <- function(data) {
    ggplot(data) +
      ...
      geom_sf(data = TApoly, fill = "grey", size = 0.1) +
      geom_sf(aes(fill = num_species), color = NA) +
      scale_fill_distiller(pallete = "YlOrBr", limits = c(0,500)) + 
      labs(title = title, x = "Longitude", y = "Latitude", fill = "Number of species") +
      coord_sf(xlim = c(-85, -60), ylim = c(-24, 14), expand = FALSE, crs = 4326) +
      annotation_scale(location = "bl", width_hint = 0.5)  +
      theme(panel.background = element_rect(fill = "lightblue"))
  ...
  # Filter out rows with richness = 0 for both plant and frugivore
  comparison_data_filtered <- comparison_data %>%
                                filter(Plant_Richness > 0 | Frugivore_Richness > 0)
  
  # Compare plant and frugivore richness
  ## Fit a linear model
  compare_richness_linear_summary <- summary(lm(Frugivore_Richness ~ Plant_Richness),
                                      data = comparison_data_filtered)
  linear_r_squared <- compare_richness_linear_summary$r.squared
  
  ## Fit an exponential model
  compare_richness_exponential_summary <- summary(lm(log(Frugivore_Richness) ~ log(Plant_Richness),
                                      data = comparison_data_filtered))
  ...
  plant_frug_richness_compare <- ggplot(comparison_data_filtered,
  ...
  labs(title = paste("Plant Richness vs Frugivore Richness [", resolution_meters/1000, " km]", sep = ""),
    ...
    geom_smooth(method = "lm", se = FALSE, color = "blue", formula = y ~ exp(x), linetype = "dashed")

  # Create histograms
  plant_richness_hist <- hist(comparison_data_filtered$PlantRichness, 
    main = paste("Histogram of Plant Richness [", resolution_meters/1000, " km] by cell", sep = ""),
                              xlab = "Plant Richness by cell")
  frugivore_richness_hist <- hist(comparison_data_filtered$FrugivoreRichness,
...
Richness maps
```{r}
all_richness_plots <- gridExtra::grid.arrange(#plantgridRichTA_1km, plantgridRichTA_5km, 
