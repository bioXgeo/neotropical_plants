---
title: "Tropical Andes Taxonomic Diversity of Plants and Frugivores"
author: "Hazel J. Anderson"
project: "Plant-Frugivore Diversity"
collaborators: "Beth E. Gerstner, Phoebe L. Zarnetske"
overview: "Calculating and mapping taxonomic diversity as species richness using occurrence data for plants and frugivores in the Tropical Andes Moist Lowland and Montane forests. Note: some code is adapted from https://luisdva.github.io/rstats/richness/."
data input: "TropicalAndes_plant_occ.csv, TropicalAndes_frugivore_occ.csv"
data output: "TropicalAndes_frugivore_richness.csv, TropicalAndes_GBIF_plant_richness.csv"
date: "2023-08-01"
output: html_document
---

# Load required packages
```{r}
library(sf)
library(dplyr)
library(ggplot2)
library(rnaturalearth)
library(smoothr)
library(purrr)
library(raster)
library(scico)
library(ggspatial)
```

# Set file paths
```{r}
data_path<-file.path('G:/Shared drives/SpaCE_Lab_FRUGIVORIA/data/plants/L1')
output_path<- file.path('G:/Shared drives/SpaCE_Lab_FRUGIVORIA/data/plants/L2')
```

# Read in Data
```{r}
TropicalAndes_plant_occ <- read.csv(file.path(data_path,"TropicalAndes_plant_occ.csv"))
TropicalAndes_frugivore_occ <- read.csv(file.path(data_path,"TropicalAndes_frugivore_occ.csv"))
```

# Polygons of countries
```{r}
worldMap <- ne_countries(scale = "medium", type = "countries", returnclass = "sf")
#polygon of Tropical Andes
TApoly <- worldMap %>% filter(sovereignt == "Bolivia" |sovereignt == "Ecuador" | sovereignt == "Venezuela" | sovereignt == "Colombia" | sovereignt == "Peru")
```

# Convert data to spatial data
```{r}
plants.sf <- st_as_sf(TropicalAndes_plant_occ, coords = c("decimalLongitude", "decimalLatitude"), crs = 4326)

frugivores.sf <- st_as_sf(TropicalAndes_frugivore_occ, coords = c("decimalLongitude", "decimalLatitude"), crs = 4326)
```

```{r}
plants_sf_species <- plants.sf %>%
  group_by(species) %>%
  summarise()

frugivores_sf_species <- frugivores.sf %>%
  group_by(species) %>%
  summarise()
```

# Plot Base Map
```{r}
basePlot <-
ggplot() +
geom_sf(data = worldMap, fill = "white") +
geom_sf(data = TApoly) +
labs(title = "Tropical Andes", x = "Latitude", y = "Longitude") +
coord_sf(xlim = c(-94, -54), ylim = c(-24, 14), expand = FALSE) +
annotation_scale(location = "bl", width_hint = 0.5) +
annotation_north_arrow(location = "bl", which_north = "true", 
  pad_x = unit(0.75, "in"), pad_y = unit(0.5, "in"), style = north_arrow_fancy_orienteering) +
theme(panel.background = element_rect(fill = "lightblue"))

basePlot
```

# Plot points
```{r}
plantsPointsPlot <-
  ggplot() +
  geom_sf(data = worldMap, fill = "white") +
  geom_sf(data = TApoly) +
  geom_sf(data = plants_sf_species, pch = 21) +
  labs(title = "Tropical Andes Plant Occurrences", x = "Latitude", y = "Longitude") +
  coord_sf(xlim = c(-94, -54), ylim = c(-24, 14), expand = FALSE) +
  theme(
    plot.background = element_rect(fill = "#f1f2f3"),
    panel.background = element_rect(fill = "lightblue"),
    panel.grid = element_blank(),
    line = element_blank(),
    rect = element_blank()
  )
plantsPointsPlot
```

```{r}
frugivoresPointsPlot <-
  ggplot() +
  geom_sf(data = worldMap, fill = "white") +
  geom_sf(data = TApoly) +
  geom_sf(data = frugivores_sf_species, pch = 21) +
  labs(title = "Tropical Andes Bird and Mammal Occurrences", x = "Latitude", y = "Longitude") +
  coord_sf(xlim = c(-94, -54), ylim = c(-24, 14), expand = FALSE) +
  theme(
    plot.background = element_rect(fill = "#f1f2f3"),
    panel.background = element_rect(fill = "lightblue"),
    panel.grid = element_blank(),
    line = element_blank(),
    rect = element_blank()
  )
frugivoresPointsPlot
```

# Make 0.5 Grid
```{r}
# make grid
TAGrid0.5 <- TApoly %>%
  st_make_grid(cellsize = c(0.5)) %>%
  st_intersection(TApoly) %>%
  st_cast("MULTIPOLYGON") %>%
  st_sf() %>%
  mutate(cellid = row_number())
```

# Plot Empty Grid
```{r}
# empty grid
gridPlot <-
  ggplot() +
  geom_sf(data = worldMap, fill = "white")+
  geom_sf(data = TApoly) +
  geom_sf(data = TAGrid0.5) +
  labs(title = "Tropical Andes Grid [0.5°]", x = "Latitude", y = "Longitude") +
  coord_sf(xlim = c(-94, -54), ylim = c(-24, 14), expand = FALSE) +
  theme(
    plot.background = element_rect(fill = "#f1f2f3"),
    panel.background = element_rect(fill = "lightblue"),
    panel.grid = element_blank(),
    line = element_blank(),
    rect = element_blank()
  )
gridPlot
```

# Cell Richness
```{r}
plant_richness_grid0.5 <- TAGrid0.5 %>%
  st_join(plants_sf_species) %>%
  mutate(overlap = ifelse(!is.na(species), 1, 0)) %>%
  group_by(cellid) %>%
  summarize(num_species = sum(overlap))
```

```{r}
frugivore_richness_grid0.5 <- TAGrid0.5 %>%
  st_join(frugivores_sf_species) %>%
  mutate(overlap = ifelse(!is.na(species), 1, 0)) %>%
  group_by(cellid) %>%
  summarize(num_species = sum(overlap))
```

# Richness plots
```{r}
plantgridRichTA0.5 <-
  ggplot(plant_richness_grid0.5) +
  geom_sf(data = worldMap, fill = "white")+
  geom_sf(data = TApoly, fill = "grey", size = 0.1) +
  geom_sf(aes(fill = num_species), color = NA) +
  scale_fill_scico(palette = "romaO", direction = 1, end = 0.9, midpoint = 1500) +
  coord_sf(xlim = c(-94, -54), ylim = c(-24, 14), expand = FALSE) +
  theme(
    plot.background = element_rect(fill = "#f1f2f3"),
    panel.background = element_rect(fill = "lightblue"),
    panel.grid = element_blank(),
    line = element_blank(),
    rect = element_blank()
  ) +
  annotation_scale(location = "bl", width_hint = 0.5) +
  annotation_north_arrow(location = "bl", which_north = "true", 
  pad_x = unit(0.75, "in"), pad_y = unit(0.5, "in"), style = north_arrow_fancy_orienteering) +
  labs(title = "Tropical Andes Plant Richness [0.5°]", x = "Latitude", y = "Longitude", fill = "Number of species")
plantgridRichTA0.5
```

```{r}
frugivoregridRichTA0.5 <-
  ggplot(frugivore_richness_grid0.5) +
  geom_sf(data = worldMap, fill = "white")+
  geom_sf(data = TApoly, fill = "grey", size = 0.1) +
  geom_sf(aes(fill = num_species), color = NA) +
  scale_fill_scico(palette = "romaO", direction = 1, end = 0.9, midpoint = 1500) +
  coord_sf(xlim = c(-94, -54), ylim = c(-24, 14), expand = FALSE) +
  theme(
    plot.background = element_rect(fill = "#f1f2f3"),
    panel.background = element_rect(fill = "lightblue"),
    panel.grid = element_blank(),
    line = element_blank(),
    rect = element_blank()
  ) +   
  annotation_scale(location = "bl", width_hint = 0.5) +
  annotation_north_arrow(location = "bl", which_north = "true", 
  pad_x = unit(0.75, "in"), pad_y = unit(0.5, "in"), style = north_arrow_fancy_orienteering) +
  labs(title = "Tropical Andes Frugivore Richness [0.5°]", x = "Latitude", y = "Longitude", fill = "Number of species")
frugivoregridRichTA0.5
```

# Extract cell values
```{r}
plant_cellRichness0.5 <- plantgridRichTA0.5[["data"]]
frugivore_cellRichness0.5 <- frugivoregridRichTA0.5[["data"]]
```

```{r}
plot(plant_cellRichness0.5$num_species, frugivore_cellRichness0.5$num_species,
     main = "Plant Richness vs Bird & Mammal Richness by cell  [0.5°]",
     xlab = "Plant Richness by cell", ylab = "Bird & Mammal Richness by cell")
abline(lm(frugivore_cellRichness0.5$num_species ~ plant_cellRichness0.5$num_species))
summary(lm(frugivore_cellRichness0.5$num_species ~ plant_cellRichness0.5$num_species))
```

```{r}
hist(plant_cellRichness0.5$num_species,
     main = "Histogram of Plant Richness by cell  [0.5°]",
     xlab = "Plant Richness by cell")
hist(frugivore_cellRichness0.5$num_species,
     main = "Histogram of Frugivore Richness by cell  [0.5°]",
     xlab = "Frugivore Richness by cell")
```

```{r}
summary(plant_cellRichness0.5$num_species)
summary(frugivore_cellRichness0.5$num_species)
```

# Write data to csv
```{r}
write.csv(plant_cellRichness0.5, file.path(output_path,"TropicalAndes_GBIF_plant_richness.csv"))
write.csv(frugivore_cellRichness0.5, file.path(output_path,"TropicalAndes_frugivore_richness.csv"))
```
