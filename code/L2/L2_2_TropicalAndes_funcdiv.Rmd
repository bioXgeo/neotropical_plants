...
# Read in Data
```{r}
TropicalAndes_plant_occ_forest <- read.csv(file.path(data_path,"TropicalAndes_plant_occ_forest_sub.csv"))
TropicalAndes_frugivore_occ_forest <- read.csv(file.path(data_path,"TropicalAndes_frugivore_occ_forest_sub.csv"))
...
```{r}
coordinates_sf_plant <- st_as_sf(TropicalAndes_plant_occ_forest_sub, coords = c("decimalLongitude", "decimalLatitude"), crs = st_crs(Forest_sf))
coordinates_sf_frugivore <- st_as_sf(TropicalAndes_frugivore_occ_forest_sub, coords = c("decimalLongitude", "decimalLatitude"), crs = st_crs(Forest_sf))
```

```{r}
# Perform the spatial intersection
intersections_plant_TA <- st_intersection(coordinates_sf_plant, Forest_sf)
intersections_frugivore_TA <- st_intersection(coordinates_sf_frugivore, Forest_sf)
...
plant_traits_df$X <-NULL
frugivore_traits_df$X <-NULL
```
...
sp_faxes_coord_plants_sub <- as.data.frame(sp_faxes_coord_plants_sub)
...
subset_matrix_plant_sub <- subset_matrix_plant %>% filter(names(subset_matrix_plant) %in% sp_faxes_coord_plants_sub_names)
```
