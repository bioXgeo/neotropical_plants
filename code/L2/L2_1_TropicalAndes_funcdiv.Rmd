...
subset_matrix_frugivore <- PAM_frugivore_site_final_100km[row_sums_frugivore >= 4, ]
```
...
alpha_fd_indices_frugivore <- mFD::alpha.fd.multidim(
  ...
  ind_vect         = "fdis",
  #did not run fdiv or fric (slows it down); c("fide", "fdis", "fmpd", "fnnd", "feve", "fori", "fspe")
  scaling          = TRUE,
  check_input      = TRUE,
...
subset_coords_frugivore <-site_loc_key_frugivore_100km[rowSums(PAM_frugivore_site_final_100km) >= 4, ]
...
create_grid <- function(data, cellsize) {
  st_make_grid(st_as_sfc(st_bbox(data)), cellsize = cellsize) %>%
    st_sf() %>%
    mutate(cellid = row_number())
}

# Aggregate functional dispersion within each grid cell
aggregate_to_grid <- function(grid, points, value_col) {
  points %>%
    st_join(grid) %>%
    group_by(cellid) %>%
    summarize(mean_value = mean(get(value_col), na.rm = TRUE)) %>%
    ungroup() %>%
    st_set_geometry("geometry")
}

# use funtion
resolution_meters <- 100000
variable_name <- "fdis_frugivore"

# Create grid
grid <- create_grid(spatial_fdis_frugivore, cellsize = resolution_meters)

# Aggregate data to grid
aggregated_data <- aggregate_to_grid(grid, spatial_fdis_frugivore, variable_name)

# Convert geometries to centroids
aggregated_data_centroids <- st_centroid(aggregated_data)

# Extract coordinates from centroids
aggregated_data_coords <- st_coordinates(aggregated_data_centroids)

# Add coordinates as separate columns
aggregated_data <- aggregated_data %>%
  mutate(x = aggregated_data_coords[, 1], y = aggregated_data_coords[, 2])

# Remove rows with NA values in mean_value
aggregated_data <- aggregated_data %>%
...
FDisplot <-
    ...
    geom_sf(data = grid, fill = "grey")+
    geom_sf(data = aggregated_data, aes(fill = mean_value), color = NA) +
    labs(x = "Longitude", y = "Latitude", fill = "FDis") +
    scale_fill_gradient(low = "blue", high = "red") +
    coord_sf(xlim = c(-82, -60), ylim = c(-24, 14), expand = FALSE, crs = 4326) +
    theme(panel.background = element_rect(fill = "lightblue")) +
    theme(text = element_text(size = 12)) # Explicitly setting text size
FDisplot
```

# Mapping without projection 
Because projecting is being difficult
```{r}
TropicalAndes_IUCNHabitat_Forest_latlong <- read_sf(file.path(data_path_L0, "Forest_sf.shp"), layer = "Forest_sf")
Americas_latlong <- ne_countries(continent = c("North America", "South America"), returnclass = "sf")
#polygon of Tropical Andes
TApoly_latlong <- Americas_latlong %>% filter(sovereignt == "Bolivia" |sovereignt == "Ecuador" | sovereignt == "Venezuela" | sovereignt == "Colombia" | sovereignt == "Peru")
```
...
FDisplot <-
    ggplot() +
    geom_sf(data = Americas_latlong, fill = "white")+
    geom_sf(data = TApoly_latlong, fill = "lightgrey", size = 0.1) +
    geom_point(data = spatial_fdis_frugivore , aes(fill = fdis_frugivore)) +
    scale_fill_gradient(low = "blue", high = "red") +
    coord_sf(xlim = c(-82, -60), ylim = c(-24, 14), expand = FALSE, crs = 4326) +
    theme(panel.background = element_rect(fill = "lightblue")) +
...
# Plants
Test with small random sample as a subset to make it work before using all data
```{r}
set.seed(100)
all_plant_species <- row.names(plant_traits_df_final)
plant_random_subset <- sample(all_plant_species, 1000)
# objects that need to be subset
PAM_plant_site_final_df <- as.dataframe(PAM_plant_site_final) #matrix
