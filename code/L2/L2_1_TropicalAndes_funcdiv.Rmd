...
```{r}
create_fdis_plots <- function(resolution_meters, data, variable_name) {
# Make Grid
TAGrid <- TApoly %>%
  ...
  # Calculate functional dispersion for grid
  fdis_grid <- TAGrid %>%
    st_join(data) %>%
    group_by(cellid) %>%
    summarize(
      sum_variable = sum(variable_name, na.rm = TRUE),
      mean_variable = mean(variable_name, na.rm = TRUE)
    )
  ...
  FDplot <-
    ...
    geom_sf(data = fdis_grid, aes(fill = fdis_frugivore)) +
    scale_fill_gradient(low = "blue", high = "red") +
    coord_sf(xlim = c(-82, -60), ylim = c(-24, 14), expand = FALSE, crs = 4326) +
    theme(panel.background = element_rect(fill = "lightblue")) +
    annotation_scale(location = "bl", width_hint = 0.5) +
...
```{r}
create_fdis_plots(100000, spatial_fdis_frugivore_projected, fdis_frugivore)
```
