...
```{r}
plant_traits <- dplyr::bind_rows(BIEN_traits_stan, TRY_traits_stan, .id = 'source')
```
...
plant_traits_dispersal <- plant_traits[plant_traits$trait_name == "Dispersal syndrome" | plant_traits$trait_name == "whole plant dispersal syndrome",]
...
# Plant height traits
```{r}
plant_traits_height <- plant_traits[plant_traits$trait_name == "Plant height vegetative" |plant_traits$trait_name == "whole plant height" ,]
...
```{r}
plant_traits_longevity <- plant_traits[plant_traits$trait_name == "Plant lifespan (longevity)" | plant_traits$trait_name == "maximum whole plant longevity",]
#trait values are a mix of numeric and characters
```

# Combine all traits
```{r}
# combine all trait wide tables
plant_traits_wide <- bind_cols(plant_traits_height_wide, plant_traits_mass_wide)
...
# Write data to csv
```{r}
write.csv(plant_traits, file.path(output_path,"TropicalAndes_plant_traits_forest.csv"))
```