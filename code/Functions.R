...
#### Standardize trait measurements ####
>>>>>>> 10d1d6fc81c06754240c73932e04f381e358f5a7
...
create_presence_absence_matrix <- function(resolution_meters, species_sf) {
  ...
  rownames(presence_absence_matrix_matrix) <- paste0(
    "cell_",
    seq_len(nrow(presence_absence_matrix_matrix))
  )

  # Ensure latitude and longitude are the first columns
  presence_absence_matrix_matrix <- presence_absence_matrix_matrix[, c(
    "Latitude",
    "Longitude",
    setdiff(
      colnames(presence_absence_matrix_matrix),
      c("Latitude", "Longitude", "cellid")
    )
  )]
...
create_rich_plots <- function(resolution_meters) {
  ...
  TAGrid_chunks <<- split(
    TAGrid,
    ceiling(seq_along(TAGrid$cellid) / chunk_size)
  )
  ...
  frugivore_richness_grid <- foreach(
    chunk = TAGrid_chunks,
    .combine = bind_rows,
    .packages = c("dplyr", "sf")
  ) %dopar%
  ...
  # Create histograms
  plant_richness_hist <- hist(
    plant_richness_grid$num_species,
    main = paste(
      "Histogram of Plant Richness [",
  ...
  frugivore_richness_hist <- hist(
    frugivore_richness_grid$num_species,
    main = paste(
      "Histogram of Frugivore Richness [",
      resolution_meters / 1000,
      " km] by cell",
      sep = ""
    ),
  ...
  mammal_richness_hist <- hist(
    mammal_richness_grid$num_species,
    main = paste(
      "Histogram of Mammal Richness [",
      resolution_meters / 1000,
      " km] by cell",
      sep = ""
    ),
  ...
  bird_richness_hist <- hist(
    bird_richness_grid$num_species,
    main = paste(
      "Histogram of Bird Richness [",
      resolution_meters / 1000,
      " km] by cell",
      sep = ""
    ),
...
fspaces_quality2 <- function(sp_grid, traits, guild) {
  ...
  assign(
    x = paste0('fspaces_quality2_', guild),
    value = fspaces_quality,
    envir = .GlobalEnv
  )
}
...
pc_coords2 <- function(fspaces_quality, traits, guild) {
  ...
  assign(
    x = paste0('sp_faxes2_coord_', guild),
    value = sp_faxes_coord,
    envir = .GlobalEnv
  )
}
...
FD_map <- function(loc_key, PAM, resolution_meters, fdis, guild) {
  ...
  if (length(common_ids) == 0) {
    stop("No matching Cell_Number values found.")
  }

  # Align both tables using match()
  subset_coords_sp <- subset_coords_sp[
    match(common_ids, subset_coords_sp$Cell_Number),
  ]
